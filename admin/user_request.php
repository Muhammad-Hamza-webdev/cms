...
if ($_SESSION['USERTYPE'] == 2 || $_SESSION['USERTYPE'] == 3) {
    if (isset($_GET['person'])) {
        $query = 'UPDATE `accounts` SET `status`=1 where `person_id`=' . $_GET['person'];
        $result = mysqli_query($con, $query);
        header('location:user_request.php?approved=1');
    }
    ?>
<!doctype html>
<html lang="en">
...
<head>
    ...
    <title>User Request | Complain Managment System Hajvery University</title>
    <!-- favicon -->
                                        ...
                                        <tbody>
                                            <?php
                                                    if ($_SESSION['USERTYPE'] == 3) {
                                                        $query = 'SELECT * FROM `persons` AS p INNER JOIN `accounts` AS a on p.`person_id`=a.`person_id` INNER JOIN departments as d on a.`department_id`=d.`department_id` WHERE a.`status`=0 AND a.`department_id`=' . $_SESSION['DEPTID'];
//                                                        die($query);
                                                    } else {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <button type="button"
                                                        onclick="return change_user(<?= $row['person_id'] ?>)"
                                            ...
                                            <tr>
                                                <td colspan="8">
                                                    <center>No Panding Requests</center>
                                                </td>
                                            </tr>
                                            <?php
                                                }
