...
if ($_SESSION['USERTYPE'] == 2) {
    if (isset($_GET['disable'])) {
        ...
        header('location:view_all_admin.php?disabled');
    }
    if (isset($_GET['active'])) {
        $query = 'UPDATE `accounts` SET `status`=1 where `person_id`=' . $_GET['active'];
        $result = mysqli_query($con, $query);
        header('location:view_all_admin.php?actived');
    }
    ?>
<!doctype html>
<html lang="en">
...
<head>
    ...
    <title>All Admin | Complain Managment System Hajvery University</title>
    <!-- favicon -->
...
<body>
                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="page-header">
                            <h2 class="pageheader-title">All Admin</h2>
                            <div class="page-breadcrumb">
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active" aria-current="page">View All Admins</li>
                                    </ol>
                                </nav>
                            </div>
                        </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example" class="table table-striped table-bordered second"
                                        ...
                                        <tbody>
                                            <?php
                                                    $query = 'SELECT * FROM `persons` AS p INNER JOIN `accounts` AS a on p.`person_id`=a.`person_id` INNER JOIN departments as d on a.`department_id`=d.`department_id` where a.`user_type`= 3';
                                                    $result = mysqli_query($con, $query);
    ...
    function turn_off(id) {
        if (confirm("Are you sure you want to Disable account?")) {
            window.window.location.href = 'view_all_admin.php?disable=' + id;
        }
    }

    function turn_on(id) {
        if (confirm("Are you sure you want to Active account?")) {
            window.window.location.href = 'view_all_admin.php?active=' + id;
        }
    }
    </script>
    <?php
